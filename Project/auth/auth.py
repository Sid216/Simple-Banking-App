...
def check_duplicate(e):
    ...
    if r:
        flash(f"The chosen {r.group(1   )} is taken.", "warning")
    else:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            result = DB.insertOne(
                "INSERT INTO IS601_Users (email, username, password, firstname, lastname) VALUES (%s, %s, %s,%s,%s)",
                email,
                username,
                hash,
                firstname,
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if is_valid:
            try:
                result = DB.selectOne(
                    "SELECT * FROM IS601_Users where email= %(email)s or username=%(email)s",
                    {"email": email},
                )
...
def profile():
    ...
    user = DB.selectOne("SELECT * FROM IS601_Users where id = %s", user_id).row
    ...
    if form.validate_on_submit():
            ...
            try:
                ...
                if result.status and result.row:
                    ...
                    if bcrypt.check_password_hash(
                        result.row["password"], current_password
                    ):
                        # update new password
                        hash = bcrypt.generate_password_hash(password)
                        try:
                            result = DB.update(
                                "UPDATE IS601_Users SET password = %s WHERE id = %s",
                                hash,
                                user_id,
                            )
                            if result.status:
        ...
        if form.validate_on_submit():
            try:  # update email, username (this will trigger if nothing changed but it's fine)
                result = DB.update(
                    "UPDATE IS601_Users SET email = %s, username = %s, firstname=%s, lastname=%s WHERE id = %s",
                    email,
                    username,
                    firstname,
                    lastname,
