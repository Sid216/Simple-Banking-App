...
WORLD_ACCOUNT_NUMBER = "000000000000"
...
def create_account():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            try:
                result = DB.insertOne(
                    """INSERT INTO IS601_S_Accounts (account_number, account_type, balance, user_id) 
                    VALUES (%s, %s, %s, %s)""",
                    account_number,
                    account_type,
                    initial_deposit,
                ...
                world_account = DB.selectOne(
                    "SELECT * FROM IS601_S_Accounts WHERE account_number=%s",
                    WORLD_ACCOUNT_NUMBER,
                ).row

                user_account = DB.selectOne(
                    "SELECT * FROM IS601_S_Accounts WHERE account_number=%s",
                    account_number,
                ).row
                ...
                DB.insertOne(
                    """
                INSERT INTO IS601_Bank_Transaction (account_src, account_dest, balance_change, transaction_type, memo, 
                ...
                """,
                    ...
                    -initial_deposit,
                    "deposit",
                    "Initial deposit to acc no " + account_number,
                    world_account["balance"] - initial_deposit,
                    datetime.utcnow(),
                    datetime.utcnow(),
                )
                DB.insertOne(
...
def my_account():
    accounts = DB.selectAll(
        "SELECT * FROM IS601_S_Accounts WHERE user_id=%s LIMIT 5", current_user.id
    ).rows
...
def account_details(account_number):
    ...
    transaction_history = DB.selectAll(
        """
    SELECT account_src, account_dest, balance_change, transaction_type, memo, expected_total, created 
    FROM IS601_Bank_Transaction WHERE account_src=%s ORDER BY created DESC
    """,
        account["id"],
    ).rows
    ...
    if start_date and end_date:
        ...
        transaction_history = [
            t for t in transaction_history if start_date <= t["created"] <= end_date
        ]

    elif start_date:
        start_date = datetime.strptime(start_date, "%Y-%m-%d")
        transaction_history = [
            t for t in transaction_history if start_date <= t["created"]
        ]

    elif end_date:
        transaction_history = [
            t for t in transaction_history if t["created"] <= end_date
        ]
    ...
    return render_template(
        ...
        paginator=Pagination(transaction_history, page, 10),
    )
...
def withdraw():
    ...
    accounts = DB.selectAll(
        "SELECT * FROM IS601_S_Accounts WHERE user_id=%s AND account_number!=%s",
        current_user.id,
        WORLD_ACCOUNT_NUMBER,
    ).rows
    ...
    if request.method == "POST" and form.validate_on_submit():
        try:
            ...
            flash("Withdrawal was successfully!", "success")

            return redirect(
                url_for("accounts.account_details", account_number=account_number)
            )
        except Exception as e:
            flash("Withdrawal was not successful!", "danger")
        ...
        if is_valid:
            try:
                if transfer_type == "other":
                    ...
                    account_dest_last4 = other_user_form.account_number_ending.data
                    account_to = DB.selectOne(
                        """SELECT account_number FROM 
                    IS601_S_Accounts a
                    JOIN IS601_Users u ON a.user_id = u.id
                    WHERE u.lastname = %s AND a.account_number LIKE %s
                    """,
                        last_name,
                        "%%%%" + account_dest_last4,
                    )
                    if not account_to.row:
                        flash(
                            f"Destination account for user with last name '{last_name}' and"
                            f" account ending '{account_dest_last4}' not found",
                            "danger",
                        )
                        return render_template(
                            "transfer.html",
                ...
                # Retrieve the account and world account objects
                from_account = DB.selectOne(
                    "SELECT * FROM IS601_S_Accounts WHERE account_number=%s",
                    src_account_number,
                ).row
                to_account = DB.selectOne(
                    "SELECT * FROM IS601_S_Accounts WHERE account_number=%s",
                    dest_account_number,
                ).row
                ...
                if from_account["balance"] < amount:
                    form = other_user_form if transfer_type == "other" else self_form
                    form.amount.errors.append(
                        f"You Can Not Transfer More Than Your Balance. Your balance is {from_account['balance']} "
                    )
                    return render_template(
                        "transfer.html",
                        self_form=self_form,
                ...
                flash("Transfer unsuccessfull!", "danger")
